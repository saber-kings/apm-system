...
import cn.saberking.jvav.apm.web.mapper.ApmStoreCategoryMapper;
...
@Slf4j
@Service
public class ApmStoreCategoryServiceImpl implements ApmStoreCategoryService {
    @Resource
    ...
    public List<ApmStoreCategory> selectApmStoreCategoryList(ApmStoreCategory apmStoreCategory) {
        return apmStoreCategoryMapper.selectApmStoreCategoryList(apmStoreCategory);
    }
    ...
    public List<ApmStoreCategory> selectApmStoreCategoryList() {
        ...
        //打印
        log.info("商品分类数据为：{}", categoryList);
        //创建分类集合预存储带层级的分类
        List<ApmStoreCategory> categories = new ArrayList<>();
        categoryList.forEach(c -> {
            if (c.getPid().equals(Constants.PRODUCT_CATEGORY_ROOT_PID)) {
                //设置父分类名称
                c.setParentName(Constants.PRODUCT_CATEGORY_ROOT_NAME);
                //加入一级分类
                categories.add(c);
                //查找二级分类
    ...
    public List<ApmStoreCategory> selectSecondCategoryList(ApmStoreCategory apmStoreCategory) {
        ...
        log.info("二级分类信息如下：{}", categoryList);
        return categories;
    }
}
